"! S/4HANA Public Cloud adapter scaffold. Bind to released journal APIs before productive use.
CLASS /fcbp/cl_glt_adapter_s4pub DEFINITION PUBLIC FINAL CREATE PUBLIC.
...
CLASS /fcbp/cl_glt_adapter_s4pub IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF mo_payload IS NOT BOUND.
      mo_payload = NEW /fcbp/cl_glt_payload_s4pub( ).
    ENDIF.
  ...
  METHOD /fcbp/if_glt_transfer_adapter~dispatch.
    ensure_bound( iv_operation = 'S4PUB_SUBMIT' is_route = is_route ).
  ...
  METHOD /fcbp/if_glt_transfer_adapter~query_status.
    ensure_bound( iv_operation = 'S4PUB_QUERY_STATUS' is_route = is_route ).
  ...
  METHOD /fcbp/if_glt_transfer_adapter~cancel.
    ...
    rs_result-error-operator_text = |S/4 Public Cloud cancel is not enabled by the scaffold: { iv_reason }.|.
  ENDMETHOD.
  ...
  METHOD /fcbp/if_glt_transfer_adapter~validate_connection.
    ...
    ELSE.
      rs_result = VALUE #(
        ...
        blocking = abap_true
        finding_code = 'S4PUB_CLIENT_NOT_BOUND'
        operator_text = 'S/4 Public Cloud adapter requires a released communication client binding.'
  ...
  METHOD /fcbp/if_glt_transfer_adapter~get_capabilities.
    rv_capabilities = 'S4_PUBLIC_CLOUD;SUBMIT;STATUS_QUERY;MATRIX_INCOMPLETE'.
  ENDMETHOD.
  ...
  METHOD /fcbp/if_glt_transfer_adapter~get_capability_matrix.
    rs_capability = NEW /fcbp/cl_glt_adapter_capability( )->/fcbp/if_glt_adapter_capability~get_by_adapter_type(
      /fcbp/if_glt_adapter_types=>c_adapter_type-s4_public_cloud ).
  ENDMETHOD.
    ...
    IF mo_client IS NOT BOUND.
      RAISE EXCEPTION TYPE /fcbp/cx_glt_adapter
        EXPORTING
          ...
          operator_text     = |Adapter operation { iv_operation } requires a released S/4 Public Cloud client binding.|.
    ENDIF.
  ENDMETHOD.
