"! STATUS_QUERY work handler. It queries target status without blind resubmission.
CLASS /fcbp/cl_glt_wh_status_qry DEFINITION PUBLIC FINAL CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    METHODS assert_status_query_allowed
      IMPORTING
    ...
    METHODS status_for_uncertain_query
      IMPORTING
        is_transfer         TYPE /fcbp/if_glt_types=>ty_transfer
      RETURNING
        VALUE(rv_status)    TYPE /fcbp/if_glt_types=>ty_status.

    METHODS status_for_pending_query
      IMPORTING
        is_transfer         TYPE /fcbp/if_glt_types=>ty_transfer
      RETURNING
        VALUE(rv_status)    TYPE /fcbp/if_glt_types=>ty_status.
...
CLASS /fcbp/cl_glt_wh_status_qry IMPLEMENTATION.
    ...
    TRY.
        ...
        IF ls_transfer-header-status_code = /fcbp/if_glt_types=>c_status-posted.
          rs_result = VALUE #(
            ...
            message_text      = 'Status query skipped because transfer is already posted.' ).
          RETURN.
        ENDIF.

        assert_status_query_allowed( ls_transfer ).
        ...
        CASE ls_query_summary-status_code.
          ...
          WHEN /fcbp/if_glt_types=>c_status-dispatched.
            IF ls_query_summary-followup_work-work_type = /fcbp/if_glt_types=>c_outbox_work_type-status_query.
              rs_result-next_action = /fcbp/if_glt_outbox_types=>c_next_action-schedule_status_query.
            ELSEIF ls_query_summary-followup_work-work_type = /fcbp/if_glt_types=>c_outbox_work_type-poll.
  ...
  METHOD assert_status_query_allowed.
      ...
      WHEN OTHERS.
        RAISE EXCEPTION TYPE /fcbp/cx_glt_error
          EXPORTING
            ...
            operator_text  = |STATUS_QUERY requires UNKNOWN_CONFIRMATION, PROCESSING, or DISPATCHED state; current status is { is_transfer-header-status_code }.|.
    ENDCASE.
  ENDMETHOD.
    ...
    IF lt_package_id IS INITIAL.
      RAISE EXCEPTION TYPE /fcbp/cx_glt_error
        EXPORTING
          ...
          operator_text  = 'Status query requires current package evidence.'.
    ENDIF.
    ...
    IF lines( lt_package_id ) > 1.
      RAISE EXCEPTION TYPE /fcbp/cx_glt_error
        EXPORTING
          transfer_id    = is_transfer-header-transfer_id
          error_category = /fcbp/if_glt_types=>c_error_category-conflict
          operator_text  = 'Status query found more than one current package for the transfer.'.
    ENDIF.
    ...
    IF cs_transfer-header-current_package_id IS NOT INITIAL
      ...
      RAISE EXCEPTION TYPE /fcbp/cx_glt_error
        EXPORTING
          ...
          error_category = /fcbp/if_glt_types=>c_error_category-conflict
          operator_text  = |Transfer current package { cs_transfer-header-current_package_id } conflicts with status-query package { iv_package_id }.|.
    ENDIF.
    ...
    IF lv_target_id IS INITIAL.
      RAISE EXCEPTION TYPE /fcbp/cx_glt_error
        EXPORTING
          ...
          operator_text  = 'Status-query policy context has no target id.'.
    ENDIF.
  ...
  METHOD query_target_status.
    ...
    IF ls_capability-supports_status_query <> abap_true.
      RAISE EXCEPTION TYPE /fcbp/cx_glt_error
        EXPORTING
          transfer_id    = is_work-transfer_id
          error_category = /fcbp/if_glt_types=>c_error_category-config
          operator_text  = |Adapter { is_route-target_adapter } does not support status query.|.
    ENDIF.

    rs_summary-status_code = is_transfer-header-status_code.
    rs_summary-message_text = 'Target status query did not prove a final outcome.'.
    ...
    LOOP AT it_attempt INTO DATA(ls_source_attempt).
      ...
      DATA(ls_attempt) = mo_evidence->start_attempt(
        ...
        iv_attempt_type = /fcbp/if_glt_types=>c_attempt_type-status_query
      ...
      CASE ls_adapter_result-outcome.
        ...
        WHEN /fcbp/if_glt_types=>c_adapter_outcome-unknown_confirmation
          OR /fcbp/if_glt_types=>c_adapter_outcome-retryable_failure.
          rs_summary-unknown_confirmation = abap_true.
          rs_summary-retryable = ls_adapter_result-retryable.
          ...
          rs_summary-status_code = status_for_uncertain_query( is_transfer ).
          RETURN.
        WHEN /fcbp/if_glt_types=>c_adapter_outcome-not_found.
          IF is_effective_context-confirmation_policy-safe_retry_after_negative_query = abap_true
             AND is_transfer-header-status_code <> /fcbp/if_glt_types=>c_status-dispatched.
            rs_summary-status_code = /fcbp/if_glt_types=>c_status-failed_retryable.
    ...
    IF lv_pending = abap_true.
      rs_summary-status_code = status_for_pending_query( is_transfer ).
      IF is_transfer-header-status_code = /fcbp/if_glt_types=>c_status-unknown_confirmation.
        rs_summary-unknown_confirmation = abap_true.
        DATA(lv_pending_query_id) = mo_retry_service->schedule_status_query(
          iv_transfer_id = is_work-transfer_id
          iv_error_id    = rs_summary-error_id ).
        rs_summary-followup_work = build_followup_work(
          is_work      = is_work
      ...
      ENDIF.
      rs_summary-message_text = 'Target status query returned pending confirmation.'.
    ELSE.
      rs_summary-status_code = /fcbp/if_glt_types=>c_status-posted.
      rs_summary-message_text = 'Target posting confirmed by status query.'.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD build_attempt_request.
    rs_request = VALUE #(
      ...
      raw_request_ref   = |STATUS_QUERY:{ is_request-package_id }:DOC:{ is_request-outdoc_id }:HANDLE:{ is_request-query_handle_type }|
  ...
  METHOD status_for_uncertain_query.
    rv_status = is_transfer-header-status_code.
    IF is_transfer-header-status_code = /fcbp/if_glt_types=>c_status-processing.
      rv_status = /fcbp/if_glt_types=>c_status-unknown_confirmation.
    ENDIF.
  ENDMETHOD.

  METHOD status_for_pending_query.
    rv_status = is_transfer-header-status_code.
    IF is_transfer-header-status_code = /fcbp/if_glt_types=>c_status-processing.
