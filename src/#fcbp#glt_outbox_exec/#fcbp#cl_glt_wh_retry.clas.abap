"! RETRY work handler scaffold. Retry safety and adapter resubmission remain policy-driven.
  ...
  METHOD /fcbp/if_glt_work_handler~handle.
    ...
    RAISE EXCEPTION TYPE /fcbp/cx_glt_error
      EXPORTING
        transfer_id    = is_work-transfer_id
        error_category = /fcbp/if_glt_types=>c_error_category-technical
        operator_text  = 'RETRY handler pipeline is not implemented in the scaffold.'.
  ENDMETHOD.
